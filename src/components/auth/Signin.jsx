...
function Signin() {
  ...
  const submitHandler = async (data) => {
    ...
    axios
      ...
      .then((res) => {
        const token = res.data.token; // Store token in a variable
        ...
        axios
          ...
          .then((userRes) => {
            if (userRes.status === 200) {
              dispatch(
                login({ userData: userRes.data.user, roles: userRes.data.role })
              );

              toast.success(res.data.msg);
              setIsFormSubmitting(false);
              setTimeout(() => {
                navigate(`/${data.role[0]}/dashboard`);
              }, 1500);
            }
          })
          .catch((err) => {
      ...
      axios
        ...
        .then((res) => {
          if (res.status === 200) {
            ...
            setTimeout(() => {
              navigate(`/${res.data.role[0]}/dashboard`);
            }, 1300);
          } else {
          ...
          <div>
            <div className="relative mt-3 w-full">
              <select
                ...
                <option value="student" className="text-gray-400">
                  Student
                </option>
                <option value="teacher" className="text-gray-400">
                  Teacher
                </option>
